/**
 * Improved CFWheels Model Generator
 * 
 * This script generates CFWheels model files based on the SQL Server SRP_tables.sql schema.
 * It includes improvements for better singularization, self-referencing relationships, and enhanced documentation.
 */
...
/**
 * Improved singularization function that handles special cases
 * and common plural forms correctly
 * 
...
function singularize(word) {
    ...
    const specialCases = {
        ...
        'Countries': 'Country',
        'ElectoralUnities': 'ElectoralUnity', 
        'IndividualPhones': 'IndividualPhone',
        'LoadDataFiles': 'LoadDataFile',
        'ListDisplayColumns': 'ListDisplayColumn',
        'ListFilterColumns': 'ListFilterColumn',
        'ListSortColumns': 'ListSortColumn'
    };
    ...
    if (word.match(/ies$/)) {
        // e.g., Communities => Community
        return word.replace(/ies$/, 'y');
    } else if (word.match(/([sx]|ch|sh)es$/)) {
        // e.g., Boxes => Box, Brushes => Brush
        return word.replace(/([sx]|ch|sh)es$/, '$1');
...
/**
 * Get a brief description of a column based on its name and type
 * 
 * @param {Object} column - The column object
 * @return {string} - A description of the column's purpose
 */
function getColumnDescription(column) {
    const name = column.name.toLowerCase();
    
    // Common column patterns
    if (name === 'id') return 'Primary key identifier';
    if (name.endsWith('id')) return `Foreign key reference to ${pascalCase(name.replace(/id$/i, ''))}`;
    if (name.match(/^(is|has)/i)) return `Boolean flag indicating ${name.replace(/^(is|has)/i, '').replace(/([A-Z])/g, ' $1').toLowerCase()}`;
    if (name === 'name') return 'Name/title of this record';
    if (name === 'comments') return 'Additional notes or comments';
    if (name.includes('date')) return 'Date/time value';
    if (name === 'createdtimestamp') return 'When this record was created';
    if (name === 'lastupdatedtimestamp') return 'When this record was last modified';
    if (name === 'createdby') return 'User who created this record';
    if (name === 'lastupdatedby') return 'User who last modified this record';
    if (name.includes('guid')) return 'Globally unique identifier';
    
    // Default description based on type
    if (column.type === 'bit') return 'Boolean flag';
    if (column.type.includes('int')) return 'Numeric value';
    if (column.type.includes('varchar') || column.type.includes('nvarchar')) return 'Text field';
    if (column.type === 'datetime') return 'Date and time value';
    
    return 'Field containing data for this record';
}
 ...
 * 
 * @param {string} tableName - The name of the table to get relationships for
 * @return {Array} - Array of relationships where this table is the child
 */
 ...
 * 
 * @param {string} tableName - The name of the table to get relationships for
 * @return {Array} - Array of relationships where this table is the parent
 */
...
/**
 * Get self-referencing relationships for a table
 * 
 * @param {string} tableName - The name of the table to check
 * @return {Array} - Array of self-referencing relationships
 */
function getSelfReferencingRelationships(tableName) {
 ...
 * 
 * @param {string} sqlType - The SQL Server data type
 * @return {string} - The corresponding CFWheels validation type
 */
 ...
 * 
 * @param {Object} table - The table object with name, primaryKey, and columns
 * @return {string} - The generated CFWheels model content
 */
function generateModel(table) {
    ...
    const selfReferences = getSelfReferencingRelationships(tableName);
    
    // Start building the model
    let content = '';
    
    // Component header with enhanced documentation
    content += `/**\n`;
    content += ` * ${modelName} Model\n`;
    content += ` * Table: ${tableName}\n`;
    content += ` * \n`;
    content += ` * Description: Represents ${tableName.toLowerCase().replace(/_/g, ' ')} in the system\n`;
    ...
    if (belongsTo.length > 0) {
        ...
        belongsTo.forEach(rel => {
            content += ` * - ${singularize(rel.parentTable)} (via ${rel.childKey})\n`;
        });
    }
    ...
    if (hasMany.length > 0) {
        ...
        content += ` * Has many:\n`;
        hasMany.forEach(rel => {
            if (rel.parentTable !== rel.childTable) { // Skip self-references
                content += ` * - ${rel.childTable} (via ${rel.childKey})\n`;
            }
        });
    }
    ...
    if (selfReferences.length > 0) {
        ...
        selfReferences.forEach(rel => {
            content += ` * - Parent/child relationship via ${rel.childKey}\n`;
        });
    }
    
    content += ` * \n`;
    content += ` * Generated with Improved CFWheels Model Generator\n`;
    ...
    content += `     * Sets up table mapping, keys, properties, validations, and relationships\n`;
    content += `     */\n`;
    ...
    if (belongsTo.length > 0) {
        ...
        belongsTo.forEach(rel => {
            ...
            // Add relation with description
            content += `        // References the parent ${parentModel}\n`;
    ...
    if (hasMany.length > 0) {
        content += `\n        // Has Many relationships\n`;
        hasMany.forEach(rel => {
            // Handle regular has-many relationships
            if (rel.parentTable !== rel.childTable) {
                // Add relation with description
                const childModel = singularize(rel.childTable);
                content += `        // Contains multiple ${rel.childTable}\n`;
    ...
    if (selfReferences.length > 0) {
        ...
        selfReferences.forEach(rel => {
            // Parent reference (belongsTo)
            content += `        // Reference to the parent ${modelName}\n`;
            content += `        belongsTo(name="parent${modelName}", foreignKey="${rel.childKey}");\n`;
            
            // Children reference (hasMany)
            content += `        // References to child ${tableName}\n`;
            content += `        hasMany(name="child${tableName}", foreignKey="${rel.childKey}");\n`;
        });
    }
    ...
    // Add property documentation with enhanced descriptions
    if (columns.length > 0) {
        content += `    /**\n`;
    ...
    content += `    /**\n`;
    content += `     * Get a string representation of this ${modelName}\n`;
    content += `     * Returns a human-readable identifier for this record\n`;
    content += `     */\n`;
    content += `    public string function toString() {\n`;
    
    // Use Name field if available, otherwise ID
    const nameField = columns.find(col => col.name === 'Name');
    const idField = columns.find(col => col.name === 'Id');
    
    if (nameField) {
        content += `        return this.Name;\n`;
    } else if (idField) {
        content += `        return "${modelName} ##" & this.Id;\n`;
    } else {
        content += `        return "${modelName} Record";\n`;
    }
...
function generateAllModels() {
    console.log(`Generating ${TABLES.length} models with improved formatting...`);
    ...
    console.log(`\nAll models generated successfully with improvements!`);
